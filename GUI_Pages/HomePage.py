...
class HomePage(BasicPage):
    ...
    def init(self):
        ...
        self.home_page_welcome_label_var.set("Welcome Marin")
        ...
        order_menu.grid(row=0, column=0, columnspan=2, padx=5, pady=5)
        ...
        self.advanced_options_button.grid(row=2, column=1, padx=35, pady=15, sticky='e')
        ...
        self.table.grid(row=1, column=0, columnspan=2, sticky="nesw", padx=5, pady=5)
    ...
    def on_buy(self):
        ...
        if not amount.isdigit():
            from tkinter import messagebox
            messagebox.showinfo("Buy error", "Amount is not number")
            return
        try:
            query = "INSERT INTO orders (user_id, shipping_id, product_id, quantity, total_amount) VALUES ({}, {}, {}, {}, {})"\
                .format(self.controller.user_info['user_id'], shipping_id, product_id, amount, str(int(amount)*int(self.get_product_price(product_id)[0][0])))
    ...
    def populate_the_table_with_all_values(self):
        ...
        if self.controller.user_info:
            query_select = self.controller.run_query(
                "SELECT o.order_id, p.product_name, p.price, o.quantity, ship.provider, ship.delivering_price, o.total_amount+ship.delivering_price, o.date_ordered from orders o, products p, shipping_methods ship "
